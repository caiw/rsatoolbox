...
function RFX_permutation(Model, userOptions)
    ...
    %     if ~usingMasks
            ...
            t_lh = t_values(1:numberOfVertex,:);
            t_rh = t_values(numberOfVertex+1:numberOfVertex * 2,:);
            tVol_lh = MEGDataStcL;
            tVol_rh = MEGDataStcR;
            ...
            mne_write_stc_file1([outputFilename, '-lh.stc'], tVol_lh);
            mne_write_stc_file1([outputFilename, '-rh.stc'], tVol_rh);
            observed_Vol_L = tVol_lh;
            observed_Vol_R = tVol_rh;

            max_t_value = zeros(1,numberOfPermutation);

            parfor perm = 1:numberOfPermutation
                
                % initially copy the data, we'll flip the signs later
                permuted_rho = all_rho;
                simulated_Vol_L = tVol_lh;
                simulated_Vol_R = tVol_rh;

                if mod(perm, floor(numberOfPermutation/40)) == 0, fprintf('\b.'); end%if

                % TODO: this can be made more efficient with repmat
                for subjectNumber = 1:nSubjects
                    % Toss a coin. `toss` should be +1 or -1 with
                    % probability 0.5.
                    toss = (rand(1) > 0.5)*2-1;
                    permuted_rho(subjectNumber,:,:) = squeeze(all_rho(subjectNumber,:,:)).*toss;
                end

                [h,p,ci,stats] = ttest(permuted_rho);
                t_values = squeeze(stats.tstat);
